...
class CarouselState extends State<Carousel> {
      ...
      if (widget.autoplay) {
        timer = Timer.periodic(widget.autoplayDuration, (_) {
          if (_controller.hasClients) {
            if (_controller.page.round() == widget.items.length - 1) {
  ...
  Widget build(BuildContext context) {
    final List<Widget> listImages = (widget.items != null &&
        widget.items.isNotEmpty)
        ? widget.items.map<Widget>(
          (netImage) {
...
//colorFilter: ColorFilter.mode(Colors.black.withOpacity(0.2), BlendMode.dstATop),
    ...
    try {
      if (_controller != null && widget.currentIndex != null) {
        _controller.animateToPage(widget.currentIndex,
